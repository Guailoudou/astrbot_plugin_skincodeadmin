...
class skinCodeAdmin(Star):
    def __init__(self, context: Context,config: AstrBotConfig):
        ...
        self.config = config
        self.userdata_file = r".\data\plugins\astrbot_plugin_skincodeadmin\user.json"
        self.groupdata_file = r".\data\plugins\astrbot_plugin_skincodeadmin\group.json"
        self.userdata = {}
        self.groupdata = {}
 ...
 #群管理部分 参考https://github.com/qiqi55488/astrbot_plugin_appreview/blob/master/main.py
    ...
    async def process_group_join_request(self, event: AstrMessageEvent, request_data):
        ...
        if self.userdata[user_id]["is_banned"]:
            await self.approve_request(flag,False,"你已被封禁，拒绝加入")
            logger.info(f"用户{user_id}已封禁，拒绝加入")
        if(self.userdata[user_id]["is_pass"]):
            await self.approve_request(flag,True)
            logger.info(f"已通过加群请求: 用户ID={user_id}, 群ID={group_id}, 验证信息={comment}")
        else:
            await self.approve_request(flag,False,"你未拥有该群白名单")
